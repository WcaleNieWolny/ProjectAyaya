...
bool
fast_yuv_frame_transform (int8_t *p_output, uint8_t *p_y_arr,
                          uint8_t *p_cb_arr, uint8_t *p_cr_arr,
                          uint8_t *p_color_transform_table,
                          struct MemCopyRange *p_ranges, size_t ranges_len,
  ...
  for (size_t i = 0; i < ranges_len; ++i)
    {
      ...
      memcpy ((void *)p_output + memCopyRange.dstOffset,
              (void *)tmp_buf + memCopyRange.srcOffset, memCopyRange.len);
    }
...
void
free_rust_vec (struct RustVec *vec)
{
  (*vec->destructor) (vec);
}

void *
external_player_init (uint8_t *p_colorTransformTable, char *filename)
{
  AVPacket *p_av_packet;
  ...
  num_bytes = av_image_get_buffer_size (pixfmt, p_codec_parm->width,
                                        p_codec_parm->height, align);

  p_sws_ctx = sws_getContext (p_codec_parm->width, p_codec_parm->height,
                              p_codec_parm->format, p_codec_parm->width,
                              p_codec_parm->height, pixfmt, SWS_BILINEAR, NULL,
                              NULL, NULL);

  // Assign appropriate parts of ren_rgb_buffer to image planes in
  // ren_pFrameRGB Note that ren_pFrameRGB is an AVFrame, but AVFrame is a
  // superset of AVPicture
  //
  // Again: What the fuck does this code do?
  if (av_image_alloc ((*p_frame_rgb).data, (*p_frame_rgb).linesize,
                      p_codec_parm->width, p_codec_parm->height, pixfmt, align)
      < 0)
    {
  ...
  player->fps = (size_t)((double_t)p_format_ctx->streams[video_stream_index]
                             ->r_frame_rate.num
                         / (double_t)p_format_ctx->streams[video_stream_index]
                               ->r_frame_rate.den);
...
ExternalVideoData
external_player_video_data (void *self)
{
  ExternalPlayer *p_player = (ExternalPlayer *)self;
  ExternalVideoData data
      = { p_player->width, p_player->height, p_player->fps };
  return data;
}

int8_t *
external_player_load_frame (void *self)
{
  ExternalPlayer *p_player = (ExternalPlayer *)self;
  ...
  while (readFrame)
    {
      if ((ret = av_read_frame (p_player->p_format_ctx, p_player->p_av_packet))
          < 0)
        {
          log_error ("AV cannot read frame");
          return NULL;
        }
      if (p_player->p_av_packet->stream_index
          == (int)p_player->video_stream_index)
        {
          ret = avcodec_send_packet (p_player->p_codec_ctx,
                                     p_player->p_av_packet);
          if (ret < 0)
            {
          ...
          while (ret >= 0)
            {
              ret = avcodec_receive_frame (p_player->p_codec_ctx,
                                           p_player->p_frame);
              ...
              sws_scale (p_player->p_sws_ctx,
                         (uint8_t const *const *)p_player->p_frame->data,
                         p_player->p_frame->linesize, 0,
  ...
  fast_yuv_frame_transform (
      output, p_player->p_frame_rgb->data[0], p_player->p_frame_rgb->data[1],
      p_player->p_frame_rgb->data[2], p_player->p_colorTransformTable,
      (struct MemCopyRange *)
          p_player->p_mem_ranges->ptr, // Super safe pointer cast
      p_player->p_mem_ranges->len, p_player->width, p_player->height);

  return output;
}

void
external_player_free (void *self)
{
  ExternalPlayer *p_player = (ExternalPlayer *)self;
