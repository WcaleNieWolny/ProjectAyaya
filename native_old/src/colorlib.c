...
signed char col_get_mc_index(struct RgbColor color){
    ...
    if(index < 128){
        ...
        return (signed char)(-129 + (index - 127));
    }
}

double col_distance(struct RgbColor c1, struct RgbColor c2)
{
    double ra = (c1.red + c2.red) / 2.0;

    int rd = c1.red - c2.red;
    int gd = c1.green - c2.green;
    int bd = c1.blue - c2.blue;
...
void col_generate_cache(){
    printf("Starting generating color cache!\n");
    fflush(stdout);
    for (int r = 0; r < 256; r++){
        for (int g = 0; g < 256; g++){
            for (int b = 0; b < 256; b++){
                col_colorCache[(g * 256 * 256) + (b * 256 ) + r] = col_get_mc_index(col_getColor(r, g, b));
            }
        }
    }
    printf("Generating color cache ended!\n");
    fflush(stdout);
}

signed char col_get_cached_index(struct RgbColor* color){
    return col_colorCache[(color->green * 256 * 256) + (color->blue  * 256 ) + color->red];
}

short col_size(){ //Short? 248 > 128 (max signed char)
    //return sizeof(COL_PRIV_MC_COLORS)/ sizeof(COL_PRIV_MC_COLORS[0]);
    // NOTE: When minecraft changes colors I will update this magic value. For now calculating this in runtime is useless.
    return 248;
}